...
export class CodebaseInputService {
  async processPathInput(inputPath: string, options?: CodebaseFilterOptions): Promise<{ name: string; files: string[] }> {
  ...
  async processRepoInput(repoUrl: string, options?: CodebaseFilterOptions): Promise<{ name: string; files: string[] }> {
    // Pseudo-code:
  ...
  async processZipInput(zipFilePath: string, options?: CodebaseFilterOptions): Promise<{ name: string; files: string[] }> {
    // Pseudo-code:
  ...
  private applyFilters(files: string[], options?: CodebaseFilterOptions): string[] {
    ...
    const docExtensions: string[] = ['.md', '.markdown', '.txt', '.doc', '.docx', '.pdf'];
    const codeExtensions: string[] = ['.js', '.jsx', '.ts', '.tsx', '.py', '.java', '.c', '.cpp', '.cs', '.go', '.rb'];

    return files.filter((file) => {
      const ext = path.extname(file).toLowerCase();
      if (docExtensions.includes(ext)) {
        if (!includeDocs) return false;
        if (docsFilters.length > 0 && !docsFilters.some((filter) => file.includes(filter))) {
          return false;
        }
        return true;
      } else if (codeExtensions.includes(ext)) {
        if (codeFilters.length > 0 && !codeFilters.some((filter) => file.includes(filter))) {
          return false;
        }
        return true;
      }

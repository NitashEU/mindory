import { SidecarResponsePosition, UserContext } from './mindcar';
import {
...
export type OptionString = { type: 'Some'; value: string } | { type: 'None' };
...
export type ConversationMessageOkay = { type: 'Ok'; data: ConversationMessage };
...
export type SidecarVariableType = 'File' | 'CodeSymbol' | 'Selection';
...
/**
 * Transforms a ModelSelection object into a sidecar model configuration format.
 *
 * @param modelSelection - The model selection configuration object containing model and provider settings
 * @returns A promise that resolves to an object containing:
 *  - slow_model: The designated slow model identifier
 *  - fast_model: The designated fast model identifier
 *  - models: Record of model configurations with context length, temperature and provider settings
 *  - providers: Array of configured provider settings
 *
 * @example
 * const modelSelection = {
 *   slowModel: "gpt-3.5-turbo",
 *   fastModel: "gpt-3.5-turbo",
 *   models: {...},
 *   providers: {...}
 * };
 * const config = await getSideCarModelConfiguration(modelSelection);
 */
export async function getSideCarModelConfiguration(
  ...
  providers: Array<{
    ...
    FireworksAI?: { api_key: string };
    GoogleAIStudio?: { api_key: string };
...
function getModelProviderConfiguration(
  providerConfiguration: ProviderSpecificConfiguration,
  _llmType: string,
) {
